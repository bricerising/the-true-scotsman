...
4. Harden outbound calls:
   ...
   - TLS verification and timeouts/cancellation (see `apply-resilience-patterns`)
5. Verify:
   - add/extend consumer-visible tests for authz and validation (`consumer-test-coverage`)
   - run any available dependency scanning / linters in the target repo
   - sanity-check logs/metrics do not include sensitive fields (`apply-observability-patterns`)
...
- Boundary resiliency: [`apply-resilience-patterns`](../apply-resilience-patterns/SKILL.md)
- Telemetry privacy: [`apply-observability-patterns`](../apply-observability-patterns/SKILL.md)
- Consumer-visible verification: [`consumer-test-coverage`](../consumer-test-coverage/SKILL.md)
- Shared auth/config primitives (when 2+ services need the same guardrails): [`shared-platform-library`](../shared-platform-library/SKILL.md)
